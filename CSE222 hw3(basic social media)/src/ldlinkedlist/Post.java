...
public class Post {
	...
	private LDLinkedList<Like> likes = new LDLinkedList();
	private LDLinkedList<Comment> comments = new LDLinkedList();
	...
	public void printInteraction(int in) {
		...
		for(int i = 0; i<comments.size(); i++) {
			if(i == comments.isMarked()) {
				continue;
			}
			if(comments.get(i)!=null) {
				if(comments.get(i).postId==in) {
					if(flag==0) {
						System.out.println("  The post has "+(this.numberOfComment()-this.lazyCommmentSize())+" comment(s)...");
					}
